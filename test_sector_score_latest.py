...
def process_symbol(symbol, name=None):
    print(f"\n[INFO] Fetching 2 years (1h interval) of data for {symbol} ({name or symbol})...")
    ...
    if isinstance(hist, pd.DataFrame) and not hist.empty:
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                ...
                # Step-back retry: try inserting each row individually
                for rec in batch:
                    try:
                        supabase.table("rvol_data").insert(rec).execute()
                        print(f"[INFO] Inserted single row for {symbol} at {rec['datetime']}")
                        inserted += 1
                    except Exception as e2:
                        print(f"[ERROR] Failed to insert single row for {symbol} at {rec['datetime']}: {e2}")
        print(f"[RESULT] Total rows inserted for {symbol}: {inserted}")
    else:
